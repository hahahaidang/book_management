module ServiceManageBook
  ...
  def func_load_approve_page(para_search_type, para_from_tf)
    if para_search_type.nil? && para_from_tf.nil?
      ...
      return Request.pending_request.paginate(:page => params[:page], :per_page => 10)
      ...
      return Request.where(conditions).where(status:0).order('created_at DESC').paginate(:page => params[:page], :per_page => 10)
    end
  end
